...
if __name__ == '__main__':
    ...
    unlabeled_ood_id_path = f'splits/ood/unlabeled_coco_for_pascal.txt'
    ...
    if rank == 0:
        ...
        all_args = {**cfg, **vars(args), 
                    ...
                    'unlabeled_ood_id_path': unlabeled_ood_id_path,
    ...
    trainset_u_ood = SemiDataset(cfg, 'train_u_ood', id_path=unlabeled_ood_id_path)
    ...
    trainsampler_u_ood = torch.utils.data.distributed.DistributedSampler(trainset_u_ood)
    trainloader_u_ood = DataLoader(trainset_u_ood, batch_size=cfg['batch_size'],
                               pin_memory=True, num_workers=1, drop_last=True, sampler=trainsampler_u_ood)
    ...
    for epoch in range(epoch + 1, cfg['epochs']):
        ...
        trainloader_u_ood.sampler.set_epoch(epoch)

        loader = zip(trainloader_l, trainloader_u, trainloader_u, trainloader_u_ood)

        for i, ((img_x, mask_x),
                (img_w, img_s1, img_s2, ignore_mask, mix1, mix2),
                (img_w_other, img_s1_other, img_s2_other, ignore_mask_other, _, _),
                ((img_w_ood, mask_ood), img_s1_ood, img_s2_ood, ignore_mask_ood, _, _)) in enumerate(loader):
            t0 = time.time()
            ...
            img_w_ood, mask_ood = img_w_ood.cuda(), mask_ood.cuda().detach()
            img_s1_ood, img_s2_ood, ignore_mask_ood = img_s1_ood.cuda(), img_s2_ood.cuda(), ignore_mask_ood.cuda()

            # CutMix images for OOD
            cutmix_img_(img_s1_ood, img_s1, mix1)
            cutmix_img_(img_s2_ood, img_s2, mix2)
            ...
            preds, preds_fp = model(torch.cat((img_x, img_w, img_w_ood)), need_fp=True)
            pred_x, pred_w, pred_w_ood = preds.chunk(3)
            _, pred_w_fp, pred_w_fp_ood = preds_fp.chunk(3)

            pred_s1, pred_s2 = model(torch.cat((img_s1, img_s2))).chunk(2)
            pred_s1_ood, pred_s2_ood = model(torch.cat((img_s1_ood, img_s2_ood))).chunk(2)
            ...
            conf_w_ood = torch.ones_like(conf_w, dtype=torch.float)
            mask_w_mixed1_ood = cutmix_mask(mask_ood, mask_w, mix1)
            mask_w_mixed2_ood = cutmix_mask(mask_ood, mask_w, mix2)
            conf_w_mixed1_ood = cutmix_mask(conf_w_ood, conf_w, mix1)
            conf_w_mixed2_ood = cutmix_mask(conf_w_ood, conf_w, mix2)
            ignore_mask_mixed1_ood = cutmix_mask(ignore_mask_ood, ignore_mask, mix1)
            ignore_mask_mixed2_ood = cutmix_mask(ignore_mask_ood, ignore_mask, mix2)
            ...
            ## Loss for OOD
            if criterion_u is not None:
                loss_s1_ood = criterion_u(pred_s1_ood, mask_w_mixed1_ood)
                loss_s1_ood = confidence_weighted_loss(loss_s1_ood, conf_w_mixed1_ood, ignore_mask_mixed1_ood, cfg)
                
                loss_s2_ood = criterion_u(pred_s2_ood, mask_w_mixed2_ood)
                loss_s2_ood = confidence_weighted_loss(loss_s2_ood, conf_w_mixed2_ood, ignore_mask_mixed2_ood, cfg)

                loss_fp_ood = criterion_u(pred_w_fp_ood, mask_ood)
                loss_fp_ood = confidence_weighted_loss(loss_fp_ood, conf_w_ood, ignore_mask_ood, cfg)
                
                weight_lambda = 0.5
                loss_ood = weight_lambda * (loss_s1_ood * 0.25 + loss_s2_ood * 0.25 + loss_fp_ood * 0.5)

            loss = (loss_ood + loss_x + loss_s1 * 0.25 + loss_s2 * 0.25 + loss_fp * 0.5) / (2.0 + weight_lambda)
            if maskclip_consistency_lambda != 0:
            ...
            log_avg.update({
                ...
                'train/loss_s_ood': (loss_s1_ood + loss_s2_ood) / 2.0,
                'train/loss_fp_ood': loss_fp_ood,
            })
            if maskclip_consistency_lambda != 0:
                log_avg.update({
