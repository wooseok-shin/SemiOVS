...
class SemiDataset(Dataset):
    def __init__(self, cfg, mode, id_path=None, nsample=None):
        ...
        if mode == 'train_u_ood':
            self.root = os.path.expandvars(os.path.expanduser(cfg['data_root_ood']))
        else:
        ...
        if mode == 'train_l' or mode == 'train_u' or mode == 'train_u_ood':
    ...
    def __getitem__(self, item):
        ...
        if self.mode == 'train_u_ood':
            ignore_mask[mask == 255] = 255
            mask = torch.where(mask == 255, 0, mask)
            return normalize(img_w, mask), img_s1, img_s2, ignore_mask, cutmix_box1, cutmix_box2
        else:
