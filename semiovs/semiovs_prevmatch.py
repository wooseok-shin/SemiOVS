...
from util.prev_utils import update_previous_list, get_previous_logits
...
def main():
    ...
    for epoch in range(epoch + 1, total_epochs):
        ...
        if is_best and (epoch >= base_only_epoch):
            update_previous_list(prev_model_list, model)
        ...
        for i, ((img_x, mask_x),
            ...
            with torch.cuda.amp.autocast(enabled=amp):
                ...
                if len(prev_model_list) != 0:
                    pred_u_w_prev = get_previous_logits(prev_model_list, torch.cat((img_u_w, img_u_w_mix)))
                ...
                if len(prev_model_list) != 0:
                    ...
                    total_loss = ((loss_x + loss_u_s1 * 0.25 + loss_u_s2 * 0.25 + loss_u_w_fp * 0.5) + loss_prev + loss_ood) / (2.0 + current_lambda + cfg['lambda'])
                else:
